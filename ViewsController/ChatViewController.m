...
#import "RTCAudioChatViewController.h"
#import "RTCVideoChatViewController.h"


#define KEYBOARD_HEIGHT     216.0f

@interface ChatDataLabel : UILabel

@end

@implementation ChatDataLabel
...
@interface ChatViewController ()<UITableViewDataSource, UITableViewDelegate, UIImagePickerControllerDelegate> {
...
- (void)dealloc
{
    ...
    // 获取当期联系人，清空未读消息
    [[NSNotificationCenter defaultCenter] postNotificationName:@"RESET_CURRENT_CONTACT_UNREAD_MESSAGES_COUNT" object:self.chatViewModel.buddyJID userInfo:nil];
    [ChatViewController setCurrentBuddyJid:nil];
}
...
- (instancetype)initWithBuddyJID:(XMPPJID *)buddyJID buddyName:(NSString *)buddyName
{
    if (self = [super initWithNibName:nil bundle:nil]) {
        [ChatViewController setCurrentBuddyJid:buddyJID];
        
        self.title = buddyName ? buddyName : buddyJID.user;
        [self.navigationItem setRightBarButtonItem:[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"buddy_header_info"] style:UIBarButtonItemStylePlain target:self action:@selector(buddyChatSet)] animated:YES];
        
        self.buddyName = buddyName;
        self.hidesBottomBarWhenPushed = YES;
        
        self.chatViewModel = [[ChatViewModel alloc] initWithModel:[[XMPPManager sharedManager] managedObjectContext_messageArchiving]];
        self.chatViewModel.buddyJID = [ChatViewController currentBuddyJid];
        
        [[XMPPManager sharedManager].xmppStream addDelegate:self delegateQueue:dispatch_get_main_queue()]; //这里的作用是当发送消息出去的时候使下一条信息可以进行发送，避免上一条还没有发送成功，又进行发送当前消息，也就是避免重复发送同一条消息。。。
        
        @weakify(self)
        [self.chatViewModel.fetchLaterSignal subscribeNext:^(id x) {
            @strongify(self)
        ...
        [self.chatViewModel.fetchEarlierSignal subscribeNext:^(NSIndexPath *indexPath) {
            @strongify(self);
            dispatch_async(dispatch_get_main_queue(), ^{
                [self.tableView reloadData];
                if (indexPath) {
...
- (void)viewDidLoad {
    ...
    [self.refreshControl addTarget:self action:@selector(fetchEarlierMessageAction) forControlEvents:UIControlEventValueChanged];
    [self.tableView addSubview:self.refreshControl];
    ...
    [self.chatViewModel fetchEarlierMessage];
    [self.tableView reloadData];
    [self scrollToBottomAnimated:YES];
}
...
{
    NSLog(@"个人聊天设置...");
    UserDetailViewController *userDetailViewController = [[UserDetailViewController alloc] initWithUser:self.buddyName];
...
{
    return [self.chatViewModel numberOfSections];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    XMPPMessageArchiving_Message_CoreDataObject *coreDataMessage = [self.chatViewModel objectAtIndexPath:indexPath];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    ChatDataLabel *dataTextView = [[ChatDataLabel alloc] initWithFrame:CGRectZero];
    dataTextView.textColor = [UIColor whiteColor];
    dataTextView.font = [UIFont systemFontOfSize:13.f];
    dataTextView.backgroundColor = [UIColor lightGrayColor];
    dataTextView.layer.cornerRadius = 4.f;
    dataTextView.layer.masksToBounds = YES;
    [bgView addSubview:dataTextView];
    
    NSString *title = [self.chatViewModel titleForHeaderInSection:section];
    dataTextView.text = title;
    [dataTextView sizeToFit];
    dataTextView.width = dataTextView.width + 4 * 2;        // 双倍边界
    dataTextView.center = CGPointMake(tableView.width / 2, bgView.height / 2);
...
- (void)scrollToLastRow
{
    [self.tableView scrollToRowAtIndexPath:
     [NSIndexPath indexPathForRow:[self.chatViewModel numberOfItemsInSection:0] inSection:0]
...
{
    NSLog(@"发送文本消息");
    [self.chatViewModel sendMessageWithText:plainMessage];
}
...
{
    [self.chatViewModel sendMessageWithText:self.chatSendBar.inputText];
}
...
{
    NSLog(@"完成语音记录,发送...");
    [self.chatViewModel sendMessageWithAudioTime:time data:voiceData urlkey:voiceName];
//    [self.chatViewModel sendMessageWithData:voiceData time:[NSString stringWithFormat:@"%ld",(long)time]];
}


#pragma mark 完成声音记录并发送BASE64编码数据
- (void)didFinishRecordingAudioWithData:(NSData *)data bodyName:(NSString *)time
{
    NSLog(@"完成语音记录");
    [self.chatViewModel sendMessageWithData:data time:time];
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    NSLog(@"获取并发送图片...");
    [self.chatViewModel sendMessageWithImage:image];            // 一对一聊天
...
{
    NSLog(@"ChatViewController 发送消息");
    NSString *buddyBar = [[message attributeForName:@"to"] stringValue];
    XMPPJID *buddyJid = [XMPPJID jidWithString:buddyBar];
    
    if ([self.chatViewModel.buddyJID isEqualToJID:buddyJid options:XMPPJIDCompareBare]) {
...
- (void)xmppStream:(XMPPStream *)sender didFailToSendMessage:(XMPPMessage *)message error:(NSError *)error
{
    NSLog(@"消息发送失败, %@", error);
    NSString *buddyBar = [[message attributeForName:@"to"] stringValue];
    if ([self.chatViewModel.buddyJID.bare isEqualToString:buddyBar]) {
        [self.chatSendBar makeSendEnable];
    }
}
